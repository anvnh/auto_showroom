...
const PostDetailed = () => {
    ...
    const {mutate: deleteComment, isPending: isDeleteComment} = useMutation({
        mutationFn: async (commentId) => {
            try {
                const res = await fetch(`/api/posts/comment/${postDetailed._id}/${commentId}`, {
                    method: "DELETE",
                });
				...
				<div className="mt-10 border border-gray-900 bg-gray-600 bg-opacity-15 rounded-2xl p-3">
					...
					<div className="flex">
						<div className="avatar">
							<Link
								to={`/social/profile/${postDetailed?.user?.username}`}
								className="w-12 h-12 rounded-full overflow-hidden"
							>
								<img
									src={
										postDetailed?.user?.profileImg ||
										placeholder_img
									}
								/>
							</Link>
					...
					{
						<div
							className={`flex flex-col gap-3 max-h-[800px] overflow-auto mt-2 rounded-2xl p-3`}
						>
							...
							{postDetailed.comments.map((comment) => (
								<div
									key={comment._id}
									className="flex gap-2 items-start bg-gray-600 bg-opacity-15 rounded-2xl p-3"
								>
									<div className="avatar">
