...
export const deleteAllCart = async (req, res) => {
    try {
        ...
        console.log("Error in deleteAllCart: ", error.message)
        res.status(500).json({error: error.message})
    }
}
...
export const sendDeliveryConfirmationMail = async (req, res) => {
    try {
        res.status(200).json({ message: "Email sent successfully" });
    } catch(error) {
        console.log("Error in sendDeliveryConfirmationMail: ", error.message)
        res.status(500).json({error: error.message})
    }
}
